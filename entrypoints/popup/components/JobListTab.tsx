    ...
    <div className="flex flex-1 flex-col overflow-hidden p-0">
      <div className="border-b p-4">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-2">
            <Button variant="destructive" size="icon" className="h-8 w-8" disabled={selectedJobs.length === 0} onClick={deleteSelectedJobs}>
      ...
      <ScrollArea type="always" className="flex-1">
        <div className="space-y-2 p-2">
          {jobs.map((job) => (
            <Card key={job.postId} className={`overflow-hidden transition-all duration-200 ${expandedJobs.includes(job.postId) ? "border-primary" : ""}`}>
              <div className="p-3">
                <div className="flex items-start gap-2">
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-start justify-between">
                      <div className="max-w-[200px] truncate font-medium">{job.postAuthor?.name || "Unknown Author"}</div>
                      <Button variant="ghost" size="icon" className="ml-auto h-6 w-6" onClick={() => window.open(`https://linkedin.com/feed/update/${job.postId}`, "_blank") }>
                        <ExternalLink className="h-3.5 w-3.5" />
                      </Button>
                    </div>
                    <p className={`text-muted-foreground text-sm ${expandedJobs.includes(job.postId) ? "" : "line-clamp-2"}`}>{job.postBody}</p>
                      ...
                      <div className="mb-3">
                        ...
                        <div className="grid grid-cols-2 gap-2">
                          {job.postContents.map((content, idx) => (
                            <a key={idx} href={content.url} target="_blank" rel="noopener noreferrer" className="hover:bg-muted block rounded border p-2">
                              <div className="flex items-center gap-2">
                                {content.type === "image" && (
                                  <div className="bg-muted-foreground/20 flex h-8 w-8 items-center justify-center rounded">📷</div>
                                )}
              ...
              <Button variant="ghost" size="sm" className="h-6 w-full rounded-none border-t" onClick={() => toggleJobExpansion(job.postId)}>
                {expandedJobs.includes(job.postId) ? <ChevronUp className="h-4 w-4" /> : <ChevronDown className="h-4 w-4" />}
              </Button>
            </Card>
          ))}
        </div>
